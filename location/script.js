window.addEventListener('DOMContentLoaded', () => {
  ...
  let oldIndex;
  let items = [];
  const button = document.querySelector('.twpx-ydw-location-btn');
  const block = document.querySelector('.twpx-ydw-location-form-control');
    ...
    try {
      ...
      if (result && typeof result === 'object') {
        if (result.status === 'success') {
          if (result.data && result.data.locationList) {
            let itemsHTML = '';
        ...
        } else if (result.errors && window.console) {
